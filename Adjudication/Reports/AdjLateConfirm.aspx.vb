...
Public Class AdjLateConfirm_Web
    ...
    Private Sub Populate_DataGrid()
        ...
        sSQL = "" & _
        " SELECT " & _
        "       Scoring.PK_ScoringID, Scoring.FK_UserID_Adjudicator, Scoring.FK_CompanyID_Adjudicator, Nominations.PK_NominationsID, " & _
        ...
        "       Production.FirstPerformanceDateTime - (SELECT TOP 1 DaysToConfirmAttendance FROM ApplicationDefaults) AS ConfirmDueDate, " & _
        ...
        " WHERE " & _
        "       (Scoring.ProductionDateAdjudicated_Planned IS NULL) " & _
        "       AND " & _
        "       (GETDATE() >= Production.FirstPerformanceDateTime - (SELECT TOP 1 DaysToConfirmAttendance FROM ApplicationDefaults)) "
        ...
        Try
            ...
            Me.lblTotalNumberOfRecords.Text = "Number of Late Confirmations: <B>" & dt.Rows.Count & "</B>"

...
Public Class ScoringFinalists_CardsForPresenters
    ...
    Protected WithEvents lblProductionCategory As System.Web.UI.WebControls.Label
    ...
    Dim SortOrder As Int16 = 1              '0 value sorts by Score, 1 sorts by Company Type (DESC) & production Title
    ...
    Private Function ReduceTableDataToTopX(ByVal dt As DataTable, Optional ByVal ByProductionType As Boolean = True) As DataTable
        ...
        If Not Session.Item("SortOrder") = Nothing Then
            If Session.Item("SortOrder") = "0" Then
                dtX.DefaultView.Sort = "ProductionCategory  DESC, " & SortWithProd & " AverageScoreFinal DESC, Title"
            Else
                dtX.DefaultView.Sort = "ProductionCategory DESC, " & SortWithProd & " Title"
            End If
        Else
            dtX.DefaultView.Sort = "ProductionCategory  DESC, " & SortWithProd & " Title"
        End If
    ...
    Public Sub SetDataGrid(ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs, Optional ByVal SeparateByProductionType As Boolean = True)
        ...
        Dim sTableString As New StringBuilder, sTxt As String = ""
        Dim sTopX As String = ""
        '====================================================================================================
        ...
        Try
            Select Case e.Item.ItemType.ToString
                ...
                Case Else
                    ...
                    If (SeparateByProductionType = True And (Not ProductionType = e.Item.Cells(2).Text)) Or _
                        (Not ProductionCategory = e.Item.Cells(1).Text) Or (Not CategoryType = e.Item.Cells(7).Text) Then
                        NumOfRows = 1       'reset on new section
                        If Not ProductionCategory = "" Then sTableString.Append("</table><hr noshade><br />")
                        ...
                        If ProductionCategory.ToUpper = ProductionType.ToUpper Then
                            sTxt = ProductionCategory.ToUpper
                        Else
                            sTxt = ProductionCategory.ToUpper & e.Item.Cells(2).Text.ToUpper
                        End If

                        sTxt = "<table cellspacing=0 cellpadding=0 style=""width:1000px; color: Black; font-family: Verdana; font-size: 11;"">" & _
                                "<tr><td colspan=99 style=""color: Black; font-family: Verdana; color: Black; font-variant: small-caps; font-family: Verdana; font-size: 12;"">" & _
                                    "...the Top " & sTopX & " Finalists for <b>Best " & CategoryName & "</b> in a " & sTxt & " Production are...<br><br> </td></tr><tr>"
                        sTableString.Append(sTxt)
                    Else
                        sTableString.Append("<tr>")
                    End If

                    sTableString.Append("<td style=""color: Black; font-family: Verdana; font-size: 12; width:100%; white-space:nowrap"" colspan=99>")

                    If BestName = BestRole Then
                        sTableString.Append("<br>&nbsp;&nbsp;&nbsp;<font style=""white-space:nowrap; font-size: 12;"" colspan=99><b>" & Title & "</b> - <i>" & CompanyName & "</i>")       'Company Name
                    Else
                        If BestRole = CategoryName Then
                            sTableString.Append("<b>" & BestName & "</b>")                                              'Best Name only
                        Else
                            sTableString.Append(" <b>" & BestName & "</b> as <i>" & BestRole & "</i>")                  'Best Name and Best Role
                        End If
                        sTableString.Append("<br>&nbsp;&nbsp;&nbsp;<font style=""white-space:nowrap; font-size: 11;"" colspan=99>" & Title & " - <i>" & CompanyName & "</i>")       'Company Name
                    End If
                    ...
                    sTableString.Append("<br><br></td><tr>")
